...
$machineid=Get-MDTComputer -macAddress $MACADDRESS001
if ($machineid.id -gt 0) 
			{
			Remove-MDTComputer $machineid.id
			}
		New-MDTComputer -macAddress $MACADDRESS001 -description $OSDComputerName -serialnumber $SERIALNUMBER -assetTag $ASSETTAG -uuid $UUID -settings @{
			OSInstall='YES';
