# SeWy: this script sets the fist PXE entry to the top, it has to contain PXE or Network in the name
# this is not working if mutliple adapters are present or IPv4 and IPv6 PXE-Options are on
# 

#$UEFIID = bcdedit /enum FIRMWARE | select-string "PXE|Network" -Context 1 -list | out-string
#$UEFIID = $UEFIID.split()
#$PXEid = $UEFIID | Select-String "{"
#bcdedit /set "{fwbootmgr}" displayorder $PXEid /addfirst
# write-host $PXEid
 
# SeWy: The following Script sets the 2. bootentry (the one that was 1. before Windows install) to be 1. again, with PXE as primary the PCs can be reinstalled remototly with WoL (WDS prestage-> allow PXE boot again)
# AKA safe method
